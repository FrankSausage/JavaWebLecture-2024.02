...
<body style="margin: 50px; margin-bottom: 100px">
	...
	<table border="1">
		<tr><th>표현식</th><th>결과</th></tr>
			...
			<tr><td>\${null + 1}</td><td>${null + 1}</td></tr> <%-- null이 0으로 변환 --%>
		...
		<tr><th colspan="2">비교 연산자</th></tr>
			...
			<tr><td>\${10 > 3}</td><td>${10 > 3}</td></tr>
			<tr><td>\${10 gt 3}</td><td>${10 gt 3}</td></tr>
		<tr><th colspan="2">논리 연산자</th></tr>
			<tr><td>\${10 >= 8 && 4 <= 3}</td><td>${10 >= 8 && 4 <= 3}</td></tr>
			<tr><td>\${10 ge 8 && 4 le 3}</td><td>${10 ge 8 && 4 le 3}</td></tr>
			<tr><td>\${!(10 >= 8 && 4 <= 3)}</td><td>${!(10 >= 8 && 4 <= 3)}</td></tr>
			<tr><td>\${not (10 ge 8 && 4 le 3)}</td><td>${not (10 ge 8 && 4 le 3)}</td></tr>
	...
	<%-- 표현식 안에서 문자열은 ' 또는 " 둘 다 사용 가능 --%>
	<h3>입력한 파라미터 값은 ${empty param.num ? '"입력하지 않았음"' : param.num} 입니다.</h3>
</body>
</html>