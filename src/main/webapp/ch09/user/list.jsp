...
<head>
	...
	<body style="margin: 50px">
		<h1> 
			사용자 목록
			<button style="margin-left: 30px" onclick="location.href='/jw/ch09/user/register'">회원 가입</button>
			<span style="font-size: 16px">
			<c:if test="${empty sessUid}">
				<button style="margin-left: 10px" onclick="location.href='/jw/ch09/user/login'">로그인</button>
			</c:if>
			<c:if test="${not empty sessUid}">
				<button style="margin-left: 10px" onclick="location.href='/jw/ch09/user/logout'">로그아웃</button>
		...
		<hr>
		<table border="1">
			<tr><th>아이디</th><th>이름</th><th>이메일</th><th>등록일</th><th>관리</th></tr>
